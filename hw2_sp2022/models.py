...
>>>>>>> 0e08b24451dae2d2b8ff97584ef76621ba89302d
...
class LogisticRegressionNewton(Model):
    ...
    def fit(self, X, y):
        ...
        # Gradient
        G = np.matmul(
            X.T, (y.reshape((samples, 1)) - sigmoid(np.matmul(X, w))))

        # Updating weights
        w = w.reshape((ft_num, 1)) - \
            (np.matmul(np.linalg.pinv(H), G)).reshape((ft_num, 1))
        self.w = w
