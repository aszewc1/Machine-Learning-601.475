...
class AgglomerativeClustering(Model):
    ...
    def fit_predict(self, X):
        ...
        while(curr_clust_num > num_clusters):
            ...
            for i in range(X.shape[0]):
                for j in range(X.shape[0]):
                    ...
                    # tie breaking
                        ...
                        old_clust1 = np.where(
                            np.array(labels) == best_i_ind)[0]
                        old_clust2 = np.where(
                            np.array(labels) == best_j_ind)[0]
                        new_key = tuple(
                            sorted((clust1.tolist()[0], clust2.tolist()[0])))
                        old_key = tuple(
                            sorted((old_clust1.tolist()[0], old_clust2.tolist()[0])))
                        if new_key < old_key:
                            best_dist = curr_dist
                            best_i_ind = i_ind
                            best_j_ind = j_ind
            ...
            labels = [new_cluster if item ==
                      old_cluster else item for item in labels]
        ...
        for i in range(num_clusters):
            if i in labels:
                ...
                labels = [i if item == max(
                    labels) else item for item in labels]
